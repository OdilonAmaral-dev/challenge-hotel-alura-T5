...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {		
					criaReserva();
					RegistroHospede registro = new RegistroHospede(ultSeqReserva, em);
        ...
        ultSeqReserva = new Long(0);
    }
	private void criaReserva() {
		var sequenciaReservaDao = new SequenciaReservaDao(em);
		var sequenciaReserva = new SequenciaReserva();
		var sequenciaReserva1 = new SequenciaReserva();

		sequenciaReserva = sequenciaReservaDao.buscarSequencia();

		if (sequenciaReserva != null) {
			ultSeqReserva = sequenciaReserva.getUltSeqReserva();
			ultSeqReserva++;
			sequenciaReserva1.setId(1l);
			sequenciaReserva1.setUltSeqReserva(ultSeqReserva);
			this.em.getTransaction().begin();
			sequenciaReservaDao.atualizar(sequenciaReserva1);
			this.em.getTransaction().commit();
		} else {
			this.em.getTransaction().begin();
			sequenciaReserva1.setId(1l);
			sequenciaReserva1.setUltSeqReserva(2023l);
			sequenciaReservaDao.cadastrar(sequenciaReserva1);
			this.em.getTransaction().commit();
			ultSeqReserva = sequenciaReserva1.getUltSeqReserva();
		}

		ReservaDao reservaDao = new ReservaDao(em);;
		var reserva = new Reserva();
		reserva.setId(ultSeqReserva);
		reserva.setDataEntrada(Objects.requireNonNull(txtDataE.getDate()));
		reserva.setDataSaida(Objects.requireNonNull(txtDataS.getDate()));
		reserva.setValor(new BigDecimal(800.00));
		reserva.setFormaPagamentoReserva(txtFormaPagamento.getSelectedItem().toString());
